...
# 단축키 
# ctrl + 2 : console 로 이도
# crrl + L : 커서 놓고 colnsole 삭제 



# Assign values to the vector A & B
A <- c(1,2,3)
B <- c(1, "A", 0.5) # 벡터는 동질형이 된다. 문자로 변경됨문자로 변경됨
...
seq(from=1.1,to=2,length=10)  # 1.1 1.2 ... 2.0
                              # 1.1 부터 2.0 까지 10등분 

rep(10,5)
rep(c(10,20,30),3)            # 세트를 통째로 3번 반복 
rep(1:3,3)                    # [1] 1 2 3 1 2 3 1 2 3
rep(c(10,20,30),each=3)       # [1] 10 10 10 20 20 20 30 30 30

x <- 1:10
x > 8
# any : 벡터의 일부 일수가 TRUE 이면 TRUE 반환 
# all : 벡터의 모든 인수가 TRUE 이면 TRUE 반환 
any(x > 8)
any(x > 20)
all(x > 8)
all(x > 0)

# NA : 물리적으로 존재하나 정확히 알 수 없는 값 
...
# 필터링 
# subset() : 해당 조건을 만족하는 원소들의 값을 반환 
# which() : 해당 조건을 만족하는 원소들의 위치(index) 를 반환
x <- c(10,20,NA,40,50)
x[x>20]
subset(x, x>20)
which(x>20)
...
# list 는 디렉토리로 생각하면 된다.
# 행렬 형태로 나타내려면 비는 부분이 있고, 여러개 벡터를 반복해서 만들 때 
...
# Data Handling: List
A <- list(name="Kang", salary = 10000, union = TRUE) # tag를 이용하여 이름을 지정가능 
A
A$name
...
C$salary <- NULL  # list 제거는 NULL 사용 
C

# list 안에 list 무한대로 생성이 가능 
...
A <- list(1:3,25:29)
A
lapply(A,median)  # list 단위 
sapply(A,median)  # 벡터 단위로
...
# dimension 설정하기 : 벡터를 matrix 화 하기
dim(A) <- c(2,3)  # matrix 가 됨 
print(A)
...
A
A[,"1st",drop=FALSE]  # 형태를 유지 
A[,"1st"]             # 형태를 풀어서 벡터로 가져옴 
...
C <- array(data=c(A,B),dim=c(3,2,2))    # dim 의 순서는 행, 열, list 순서이
C


# Part 1-4: Data Handling (Factor) ----------------------------------------임

# 열거형 값들로 이루어진 벡터 
# Character 보다는 factor 가 공간을 덜 차지함 
# Example of a factor
A <- c("Cho","Kim","Kang")
B <- as.factor(A)
...
tapply(ages, affils, mean)    # affils 기준으로 mean 을 함 
...
tmp
tapply(tmp$income, list(tmp$gender, tmp$over25), mean)  # 두가지 변수에 대해서도 가능함

split(tmp$income, list(tmp$gender, tmp$over25))  # 각각의 경우에 대해서 출력해줌 
...
C <- data.frame(A,B, stringsAsFactors=FALSE)  # 기본은 character 로 들어감 
C
C[[1]]
C[[2]]
C[1,2]
...
udcorr <- function(x,y) {
  ...
  return(mean(ud[[1]] == ud[[2]]))   # 0,1 중에 1인 비율을 추출함 true/false
}

temp <- c(10, 15, 13, 17, 20)
pressure <- c(900, 920, 890, 940, 920) # 방향성. x 가 증가할 때 y 도 증가하는가.
...
# 그림을 파일로 저장하기: pdf 형식 
pdf("Hist_dist.pdf")
pdf(paste0("Hist_dist",i,".pdf"))
hist(samp, 20, prob=T)
lines(density(samp))
dev.off()

# 보다 다양한 그래프 생성을 위해 ggplot2 패키기 이용 
# www.ggplot2.org 를 참조 
